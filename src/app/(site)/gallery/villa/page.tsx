  ...
  const cards = useMemo(
    () =>
      Array.from({ length: 10 }, (_, i) => {
        const index = i + 1;
        return {
          folder: `GalleryVilla${index}`,
          sliderFolder: `GalleryVilla${index}Extended`,
          title: t[`galleryVillaCard${index}Title` as keyof typeof t] as string,
          text: t[`galleryVillaText${index}` as keyof typeof t] as string,
          expanded: t[
            `galleryVillaTextExpanded${index}` as keyof typeof t
          ] as string,
        };
      }),
    [t],
  ...
  return (
    <div>
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-6 mt-6 space-y-2">
        ...
        <p className="text-base text-foreground/70">{t.galleryVillaIntro}</p>
      </section>
...
export default GalleryVillaPage;
